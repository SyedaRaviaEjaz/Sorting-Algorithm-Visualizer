...
let smoothAnimationFrameSaver = []; 

let randNum = () => ~~(Math.random() * 400) + 30;
function generateArr(count) {
  for (let i = 0; i < count; i++) stapleArr.push(randNum());
}

function frameDuoColorChanger(
  arr,
  indexOne,
  indexTwo,
  stapleColor,
  ...
  if (isRainbowModeOn) {
    for (let i = 0; i < arr.length; i++) {
      frame.push([
        arr[i],
        frameSaveArray[frameSaveArray.length - 1][i][1],
        i == indexOne || i == indexTwo ? strawBool : false,
      ]);
    }
  } else {
    for (let i = 0; i < arr.length; i++)
      frame.push([
        arr[i],
        sortedAlready.includes(i) ? "#00cc00" : "#0000ff",
        strawBool,
      ]);
...
let stapleWidth = ~~(staplePerimeter * 0.8);
let isStapleWideEnoughForNumberDisplay = 30 < stapleWidth;
let stapleMidForText = bars + ~~(stapleWidth / 2);
...
function createArrayAndDrawIt(exp) {
  ...
  stapleWidth = ~~(staplePerimeter * 0.8);
  isStapleWideEnoughForNumberDisplay = 30 < stapleWidth;
  generateArr(1 << exp);

  bars = (width - staplePerimeter * stapleArr.length) >> 1;
  stapleMidForText = bars + (stapleWidth >> 1);

  if (isRainbowModeOn)
    frameSaveArray.push(generateArrForRainbow(stapleArr, ~~(360 / (1 << exp))));
...
function radixSort(arr) {
  ...
  function foga(t, arr, j) {
    for (let a of t) for (let i = 0; i < a.length; i++) arr[j++] = a[i];
  }
...
function switchPlayButton(isOn) {
  document.getElementById("playToggler").className = isOn
    ? "fa fa-pause"
    : "fa fa-play";
}
...
function turnOnOff(isInvokedFromToggler) {
  ...
  function forLoop(i) {
    ...
    if (smoothAnimationFrameSaver.includes(i) && onOff) {
      let elem =
        smoothAnimationFrameSaver[smoothAnimationFrameSaver.indexOf(i) + 1];
  ...
  if (!isLastDrawOn) setTimeout(forLoop, frameSpeed, val1);
}
...
function draw(arr, ind) {
  ...
  for (
    let i = 0;
    i < stapleArr.length * staplePerimeter;
    i += staplePerimeter
  ) {
    elem = arr[Math.round(i / staplePerimeter)]; // CAUTION THIS
    ctx.fillStyle = elem[1];
    ctx.fillRect(i + bars, height - elem[0], stapleWidth, elem[0]);

    if (isRainbowModeOn && elem[2])
      rainbowCompareSticks(
        arr,
        Math.round(i / staplePerimeter),
        Math.round(i / staplePerimeter)
      );
  }
...
function smoothAnimation(arr, i1, i2) {
  ...
  function forLoop(i, index) {
    ...
    ctx.clearRect(i + bars, 0, staplePerimeter, height);
    ctx.fillRect(
      i + bars,
      height - arr[index][0],
      stapleWidth,
    ...
    if (isStapleWideEnoughForNumberDisplay)
      applyText(arr[index][0].toString(), i, height - arr[index][0], "#0000ff");
    if (index < i2)
      setTimeout(forLoop, frameSpeed / 2, i + staplePerimeter, index + 1);
    else {
      isLastDrawOn = false;
      val1++;
...
function lastDraw(arr) {
  ...
  function forLoop(i) {
    ...
    ctx.clearRect(i + bars, 0, staplePerimeter, height);
    ctx.fillRect(i + bars, height - stapleSize, stapleWidth, stapleSize);
    if (isStapleWideEnoughForNumberDisplay)
      applyText(stapleSize.toString(), i, height - stapleSize, "#00cc00");
    if (i < (stapleArr.length - 1) * staplePerimeter)
      setTimeout(forLoop, frameSpeed / 5, i + staplePerimeter);
    else resetButton();
  }
  if ((algoInd <= 1 || algoInd == 4) && !isRainbowModeOn)
...
let placeStaple = (returnArr, arr, s, ind) =>
  returnArr[ind] == undefined
    ? ind
    : placeStaple(returnArr, arr, s, arr.indexOf(s, ind + 1));
...
function changeAlgorithm(chosenAlgoInd) {
  if (algoInd != chosenAlgoInd && !onOff) {
    ...
    for (let i = 0; i < 5; i++)
      document.getElementsByClassName("order")[i].className =
        "order" + (i == algoInd ? " active" : "");
...
function createPrevArray(len) {
  ...
  for (let i = 0; i < len; i++)
    frame[i] = [prevFrame[i][0], prevFrame[i][1], prevFrame[i][2]];
...
function quickSort(arr) {
  ...
  function partition(arr, low, high) {
    ...
    for (let j = low; j < high; j++) {
      ...
      if (arr[j] <= pivot) {
        ...
        if (isRainbowModeOn) frameSaveArray.push(rainbowSwitchStaples(i, j));
          ...
          frameSaveArray.push(
            frameDuoColorChanger(arr, i, j, "#cc00ff", false)
          );
      }
    }
    ...
    if (isRainbowModeOn) {
      ...
      frameSaveArray.push(
        frameDuoColorChanger(arr, high, i + 1, "#cc00ff", false)
      );
...
function heapSort(arr) {
  ...
  function heapifyElement(arr, n, idx) {
    ...
    if (l < n && arr[largestIndex] < arr[l]) {
      frameSaveArray.push(
        frameDuoColorChanger(arr, l, largestIndex, "#ff0000", true)
      );
      largestIndex = l;
    }
    ...
    if (largestIndex != idx) {
      ...
      if (isRainbowModeOn)
        frameSaveArray.push(rainbowSwitchWithoutLogic(idx, largestIndex));
      else
        frameSaveArray.push(
          frameDuoColorChanger(arr, l, largestIndex, "#cc00ff", false)
        );
  ...
  while (!sorted && 0 < unsortedLen--) {
    ...
    for (let i = 0; i < unsortedLen; i++) {
      ...
      if (arr[i + 1] < arr[i]) {
        ...
        if (isRainbowModeOn)
          frameSaveArray.push(rainbowSwitchWithoutLogic(i + 1, i));
        else
          frameSaveArray.push(
            frameDuoColorChanger(arr, i, i + 1, "#cc00ff", false)
          );
    ...
    frameSaveArray.push(
      frameDuoColorChanger(arr, unsortedLen, unsortedLen, "#00cc00", false)
    );
